...
class _ServerViewState extends State<ServerView> {
  ...
  void initState() {
    ...
    setState(() {
      ...
      if (storage.containsKey(cStoredServers)) {
        String data = storage[cStoredServers];
        List<ServerInfo> newList = List<ServerInfo>();
        for (String json in data.split("\t")) {
          if (json.length > 0) {
            try {
