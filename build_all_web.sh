...
DIST_FOLDER="./dist"
DIST_NAME="spicecompanion-web-$(date +%y)-$(date +%m)-$(date +%d).zip"
DIST_COMMENT=${DIST_NAME}$'\n'"$GIT_BRANCH - $GIT_HEAD"$'\nThank you for playing.'

# build android
rm -rf build
webdev build
rm -rf ./build/packages/\$sdk

# copy to outdir
rm -rf ${OUTDIR}
mkdir -p ${OUTDIR}
cp ./build/index.html ${OUTDIR}
cp ./build/main.dart.js ${OUTDIR}
cp -r ./build/assets ${OUTDIR}
cp -r ./build/packages ${OUTDIR}
