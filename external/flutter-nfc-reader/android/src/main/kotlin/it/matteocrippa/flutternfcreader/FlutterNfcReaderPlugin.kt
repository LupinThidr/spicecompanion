...
class FlutterNfcReaderPlugin(val registrar: Registrar) : MethodCallHandler, EventChannel.StreamHandler, NfcAdapter.ReaderCallback {
    ...
    // active NFC technologies
    private var READER_FLAGS = NfcAdapter.FLAG_READER_NFC_A or NfcAdapter.FLAG_READER_NFC_B or NfcAdapter.FLAG_READER_NFC_F or NfcAdapter.FLAG_READER_NFC_V or 128
    ...
    override fun onTagDiscovered(tag: Tag?) {
        ...
        val data = mapOf(kId to id, kContent to null, kError to "", kStatus to "read")

        // post the read ID to the event stream
        // this must be run from the UI thread or flutter throws an exception
